...
LifeHub hat aktuell **30+ Markdown-Dateien** in `docs/`, `features/`, plus 6 Root-Master-Specs und 19 AGENTS.md-Dateien. DOX (siehe Root-`AGENTS.md`) regelt **welche** Datei wann gelesen wird — aber nicht **wie effizient** ein Agent den Inhalt durchsucht.
...
- **`docs/`-Master-Specs** — graphify ist derived state, die Specs sind Source of Truth
...
Für LifeHub-Korpus (30 .md + später Code + Bilder) wird eine **30–80× Reduktion** erwartet, sobald der Korpus wächst. Bei aktuell 30 Spec-Dateien ist der Effekt hauptsächlich **Strukturklarheit**, weniger Kompression — der Nutzen skaliert mit dem Korpus.
...
DOX-Lesepflicht für `docs/GRAPHIFY.md` selbst: keine.
