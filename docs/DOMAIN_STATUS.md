...
| **pages** | **IMPLEMENTED** | **Notion-Editor (BlockNote) 2026-09: Doc-Modell in pages.content (JSONB, Migration 0023), Legacy-Blöcke → Doc-Synthese persist-on-read (pages-doc.ts), PUT /pages/:id/doc mit ratenlimitierten Version-Snapshots, Suche über jsonb Text-Extraktion, Markdown-Export aus Doc, Version-Restore schreibt Doc zurück. Custom-Blocks: browserEmbed, researchWorkspace, lifehubImage, gallery, lifehubTable, bookmark, embed, video, file, map, pageReference, search, timeline, toggle, callout, divider. Editierbarer Titel, Detail-Route /pages/[slug] (UUID|Slug), Baum mit Einklappen, Pins-Manager, Toolbar (Versionen/MD/JSON-Export). Legacy page_blocks-Endpunkte bleiben erreichbar. Verträge: features/pages.AGENTS.md.** |
