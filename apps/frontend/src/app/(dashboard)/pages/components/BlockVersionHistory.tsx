...
export function BlockVersionHistory({ pageId, blockId, onClose }: BlockVersionHistoryProps) {
  const queryClient = useQueryClient();

  const { data: versions, isLoading } = useQuery({
    queryKey: ['block-versions', pageId, blockId],
    queryFn: () => api.get<BlockVersion[]>(`/pages/${pageId}/blocks/${blockId}/versions`),
  });

  const restoreMutation = useMutation({
    mutationFn: (version: number) =>
      api.post(`/pages/${pageId}/blocks/${blockId}/versions/${version}/restore`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['page', pageId] });
      queryClient.invalidateQueries({ queryKey: ['block-versions', pageId, blockId] });
      onClose();
    },
  });
  ...
  const getChangeTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'moved': return 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300';
      case 'archived': return 'bg-zinc-100 text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300';
