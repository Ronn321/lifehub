...
- Video-Tiles (Galerie + Album) nutzen `VideoPreviewTile` (`components/VideoPreviewTile.tsx`): Thumbnail-Bild (ffmpeg-Frame via Thumbnail-Endpoint) im Ruhezustand, das `<video>` wird NUR bei Hover gemountet (preload metadata + Seek auf Mitte + 5s-Loop via `onTimeUpdate`-Window). Kein Dauer-Mount, kein Warmup — skaliert auf 200+ Kacheln.
- Thumbnail-Endpoint: `GET /api/v1/media/files/:id/thumbnail?size=64..1024&token=` — Token-Auth wie Stream (img-Tags können keine Header senden). Bilder → sharp WebP q80 (max-edge 512), Videos → ffmpeg-Frame aus der Mitte (file.duration/2, Fallback 60s). Disk-Cache `/data/storage/thumbs/<fileId>_<size>.<ext>`, Response `Cache-Control: public, max-age=31536000, immutable`.
- Galerie-Kacheln laden NUR Thumbnails (`getThumbnailUrl(fileId, 512)` aus `lib/media.ts`) — Original-Streams (`getMediaStreamUrl`) bleiben Lightbox/Widget vorbehalten. `listFiles`/`findAlbumMedia` projizieren OHNE `thumbnailPath` (base64-data-URIs machten den Payload MB-groß — 2026-08 gefixt).
- Galerie-Pagination: `GET /media/files?sourceId=&favorite=&limit=&offset=` liefert `{ items, total }` (favorite-Filter server-seitig). GalleryTab nutzt Seiten (50/100/200, localStorage `lifehub-media-page-size`), `placeholderData` (keepPreviousData) für flüssige Seitenwechsel, Prefetch von `page±1` via `queryClient.prefetchQuery`.
- Lazy-Loading: `LazyMediaTile` (IntersectionObserver + sequentielle Aktivierungs-Queue `activatedUpTo` von oben nach unten, 4s-Timeout-Fallback). Kachel-Wrapper nutzen `[content-visibility:auto]` + `[contain-intrinsic-size:320px]` für flüssiges Rendering bei 200+/Seite; Bilder `decoding="async"`.
