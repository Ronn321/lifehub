...
auth_key = json.loads(r.stdout)['accessToken']
bearer_line = 'Authorization: Bearer *** + auth_key

r2 = subprocess.run(['curl', '-s', 'http://localhost:3007/api/v1/jellyfin/items?limit=3',
    '-H', bearer_line], capture_output=True, text=True)
...
print('Direct JF (audioCodec=aac):', r3.stdout[:400])

# Backend proxy
r4 = subprocess.run(['curl', '-s', '-D', '-',
    f'http://localhost:3007/api/v1/jellyfin/items/{items[0]["id"]}/stream?token={auth_key}',
    '-o', os.devnull], capture_output=True, text=True)
