...
import { Loader2, Plus, ExternalLink, Pin, Trash2, FolderOpen, Search } from 'lucide-react';
...
export function ResearchWorkspaceBlock({ pageId, content, onChange }: ResearchWorkspaceBlockProps) {
  ...
  const [activeTab, setActiveTab] = useState<'sources' | 'collections' | 'notes'>('sources');
  ...
  return (
    <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 overflow-hidden">
      ...
      <div className="flex border-b border-zinc-200 dark:border-zinc-800">
        {(['sources', 'collections', 'notes'] as const).map(tab => (
          <button
            key={tab}
          ...
          >
            {tab === 'sources' ? 'Quellen' : tab === 'collections' ? 'Sammlungen' : 'Notizen'}
          </button>
        ))}
      </div>
