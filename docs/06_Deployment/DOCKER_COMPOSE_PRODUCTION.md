...
> **Hinweis zu ChatGPT-Original:** Die nachfolgende Konfiguration übernimmt die Grundstruktur aus dem ChatGPT-Vorschlag und ergänzt sie um alle in `docs/01_Architecture/ARCHITECTURE.md`, `docs/04_Database/DATABASE_SCHEMA.md` und `docs/06_Deployment/TECH_STACK.md` verbindlich festgelegten Komponenten: Tailscale-Sidecar, PgBouncer-Connection-Pooling, Meilisearch, BullMQ-Worker, AES-256-Vault-Storage, separates Frontend/Backend-Image, Traefik-Security-Middleware, Let's Encrypt TLS, Health-Checks und Backup-Service mit `restic` + S3-Offsite.
