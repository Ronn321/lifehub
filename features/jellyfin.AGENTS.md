...
- **Watchlist-UX (Robert-Vorgabe):** Icon-Klick = Direkt-Toggle bei genau 1 Liste / Picker bei mehreren / Remove-aus-allen bei Mitgliedschaft; Rechtsklick = immer Picker (`WatchlistPicker`); Episoden per Rechtsklick-Kontextmenü hinzufügbar (Serien-Seite)
- **Weiterschauen-Deep-Links:** Episode-Karten linken auf `/jellyfin/series/{SeriesId}?season={ParentIndexNumber}&episode={Id}` (Serien-Seite selektiert Staffel + markiert Folge per `ep-{id}`-Scroll); Filme auf Detailseite. Resume via `UserData.PlaybackPositionTicks` → Player-Seek beim ersten canplay
- **Vollbreite ist Default** (`jellyfin-layout-store` fullWidth=true + persist-Migration); Wrapper gleicht auch oben aus (`-mt`) → Hero randlos; Hero-Höhe Vollbreite `78vh`
- **Musik-Favoriten & Queue:** Favoriten-Toggle läuft gegen `POST /jellyfin/servers/{serverId}/items/{trackId}/favorite` (Hook `useToggleFavoriteSong`, Invalidation `music-favorites`); Queue-Button setzt `nowPlayingTab='queue'` im Store + öffnet Panel; „Zuletzt gehört" = Recent-Endpoint + Invalidation `music-recent` nach Playback-Stop
